...
#'
#' @param rma_data An object of type data.frame that contains the data for the repeated measures ANOVA. Each row should represent one entity and each column one variable.
...
#' @param print_plot Logical. If TRUE, function returns a plot that displays unadjusted vs. adjusted confidence intervals. Default is TRUE.
#'
...
rma_ci = function(rma_data, C_level = 0.95, id = 1, print_plot = TRUE) {
  ...
  ci_plot = ggplot(data = ci_plot_data, aes(Me, Flm)) +
    ...
    labs(x     = "Condition",
         y     = "Value",
         title = "Comparison of adjusted and unadjusted (standard) confidence intervals") +
    theme_bw() +
