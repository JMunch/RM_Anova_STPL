...
#' @examples
#'
#'rma_sse_reduct(some_rma_data, id = 1, plot_type = "pie", return_anova_table = TRUE)
#'
#' @rdname rma_sse_reduct
#' @export


##### Evaluate reduction of error sum of squares due to the consideration of variance between entities. Function 'rma' is required!
...
rma_sse_reduct = function(rma_data, id = 1, plot_type = "pie", return_anova_table = FALSE) {
  ...
  # Note that the one-way ANOVA without repeated measures is for illustration purposes only since the data structure is correlated
  ...
  comp_plot_bar = ggplot(comparison_data, aes(model, var_percent, fill = source)) +
    ...
    labs(x     = "Model",
         y     = "Sum of squares (error)",
         title = "Reduction of sum of squared errors (SSE)") +
  ...
  comp_plot_pie = ggplot(comparison_data, aes(x = "", y = var_percent, fill = source)) +
    ...
    labs(x     = "",
         y     = "",
         title = "Reduction of sum of squared errors (SSE) in percent") +
    guides(fill = guide_legend(title = NULL)) +
    scale_fill_manual(values = c("orange", "navyblue")) +
    coord_polar(theta = "y") +
    facet_grid(~model) +
