...
# I was sure how to partition the codes into function or should it be one function??????????????????????????? JM



mauchly = function(rma_data){
...
# Create summary table for mauchly's test ---------------------------------
  ...
  return(list("mauchly_test" = mauchly_table, "corrected_Anova_Table" = ANOVA_table))
}
...
mau$corrected_Anova_Table
mau$mauchly_test
# Mauchly correction+
#mauchly_correction_factors(rma_data)
