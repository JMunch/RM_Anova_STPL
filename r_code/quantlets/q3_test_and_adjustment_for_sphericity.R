...
rma_spheri = function(rma_data, id = 1, append = FALSE) {
    ...
    mauchly_table = data.frame(check.names = FALSE, Source = "Factor", `Mauchly's W` = w, `Chi square` = chi_sq_w, df = df_w, p = p_w)
    ...
    # rmanova factor F-test (Box, 1954; Geisser & Greenhouse, 1958; Greenhouse & Geisser, 1959; Huynh & Feldt, 1976) )
    ...
    # The adjustment is realised via correction of the F-distribution degrees of freedom by
    ...
    epsilon_table = data.frame(check.names = FALSE, Source = c("Epsilon", "Adjusted p-Value"), `Lower-Bound correction (Greenhouse & Geisser, 1959)` = c(epsilon_lb, 
        p_factor_lb), `Box correction (Geisser & Greenhouse, 1958)` = c(epsilon_gg, p_factor_gg), `Huynh-Feldt correction (Huynh & Feldt, 1976)` = c(epsilon_hf, 
        p_factor_hf))
    rownames(epsilon_table) = NULL
        
        
    # Coose recomendet adjustment and add to anova-table ---------------------- 
    # Correction is only applied if assumption of sphericity is rejected Recomendation (e.g.
    ...
    # If append is 'TRUE' the function will also return the anova-table with the recomendet
