...
# Load all quantlets
source("quantlets/q1_one_way_rmANOVA.R")
source("quantlets/q2_evaluate_reduction_of_sse.R")
source("quantlets/q3_test_and_adjustment_for_sphericity.R")
source("quantlets/q4_adjusted_and_unadjusted_CI.R")
source("quantlets/q5_effect_size.R")
source("quantlets/q6_orthogonal_polynomial_contrasts.R")
source("quantlets/q7_simulate_one_way_rmANOVA_data.R")
...
rma_data[,2:5] = (rma_data[,2:5]  + 100) / 2
#ow_rma_data = read.csv("r_code/simulated_rigelmann_data.csv")
rma(rma_data)
rma_ci(rma_data, C_level = 0.99)
rma_eta(rma_data)
rma_opc(rma_data)
rma_spheri(rma_data, append = TRUE)
rma_sse_reduct(rma_data, return_anova_table = TRUE)


#rma_data = sim_rma_data(n = 30, k = 5, means = means, poly_order = NULL, noise_sd = c(155, 65, 75, 15, 40), between_subject_sd = 60, NAs = 2) 
#rma_data = (rma_data  + 100) / 2
rma_data = read.csv(file = "simulated_rigelmann_data.csv")[,-2]
...
ggsave(file = "ci_plot.pdf", plot = cis$ci_plot, device = "jpg")
...
# Data structure
data_structure = head(rma_data)
xtable(data_structure)
